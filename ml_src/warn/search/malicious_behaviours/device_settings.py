...
def detect_log(x) :
    ...
    structural_analysis_results = structural_analysis_search_method("Landroid/util/Log", "d|e|i|v|w|wtf", x)  #正则表达式匹配
    ...
    for registers in data_flow_analysis(structural_analysis_results, x):
        if len(registers) >= 2 :
            ...
            if isnt_all_regs_values([tag,message]) :
                ...
                local_formatted_str_cn = "记录了标签%s下的消息%s" % (message, tag)
...
def detect_get_package_info(x) :
    ...
    for registers in data_flow_analysis(structural_analysis_results, x):
        if len(registers) >= 2 :
            ...
            local_formatted_str_cn = "检索了%s应用程序的%s信息" % (flags, package_name)
            if not(local_formatted_str_cn in formatted_str) :
                formatted_str.append(local_formatted_str_cn)
...
def detect_package_manager_calls(x) :
    ...
    method_listing_cn = [
        ("getInstalledPackages", "读取设备上所安装的应用程序"),
        ("getSharedLibraries", "读取设备上的共享库")
    ]
